  ...
  return (
    <div className="relative w-full min-h-screen overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        <motion.div key={location.pathname} className="relative z-10 min-h-screen">
          <Routes location={location}>
            <Route path="/" element={<Welcome />} />
            <Route path="/challenge" element={<Challenge onUnlock={() => setIsUnlocked(true)} />} />
            
            {/* Protected Routes */}
            <Route path="/letter" element={isUnlocked ? <Letter /> : <Navigate to="/challenge" replace />} />
            <Route path="/moments" element={isUnlocked ? <Moments /> : <Navigate to="/challenge" replace />} />
            <Route path="/music" element={isUnlocked ? <Music /> : <Navigate to="/challenge" replace />} />
            <Route path="/details" element={isUnlocked ? <Details /> : <Navigate to="/challenge" replace />} />
            <Route path="/final" element={isUnlocked ? <Final /> : <Navigate to="/challenge" replace />} />
            <Route path="/farewell" element={isUnlocked ? <Farewell /> : <Navigate to="/challenge" replace />} />
    ...
    <>
      ...
      <div className="md:hidden flex flex-col items-center justify-center min-h-screen bg-[#f4f1ea] p-8 text-center text-[#3a352d]">
        ...
        <p className="font-sans text-lg text-[#5c554b]">
          Para disfrutar de todos los detalles e interacciones de esta carta, por favor ábrela en un computador o portátil. ¡Vale la pena!
        </p>
      </div>
      ...
      <div className="hidden md:block font-sans text-ink-dark antialiased min-h-screen">
        <Router>
          <AnimatedRoutes isUnlocked={isUnlocked} setIsUnlocked={handleUnlock} />
        </Router>
      </div>
    </>
  )
